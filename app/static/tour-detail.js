...
function setProductDetailFromParams() {
    ...
    let prodKey = Object.keys(PRODUCT_MAP).find(k => {
        ...
        return mapTitle === title && mapLoc === loc && (mapAdult === priceNorm || mapChild === priceNorm);
    });
    ...
    if (title && prod) {
        if(document.getElementById('productTitle')) document.getElementById('productTitle').innerText = title;
        if(document.getElementById('productLoc')) document.getElementById('productLoc').innerHTML = location ? `<i class='fa-solid fa-location-dot'></i> ${location}` : '';
        if(document.getElementById('productDesc')) document.getElementById('productDesc').innerText = prod.desc;
        ...
        if(document.getElementById('productPrice') && mainPriceType) {
            document.getElementById('productPrice').innerText = prod[mainPriceType].toLocaleString();
        }
        // 나머지 가격도 표시
        ...
        if (adultPriceDisplay) adultPriceDisplay.innerText = prod.adult.toLocaleString() + '원';
        if (childPriceDisplay) childPriceDisplay.innerText = (prod.child > 0) ? prod.child.toLocaleString() + '원' : '예약 불가';
        updateTotalPrice();
    } else {
        // 매칭 실패 시 기본값
        if(document.getElementById('productTitle')) document.getElementById('productTitle').innerText = title || '';
        if(document.getElementById('productLoc')) document.getElementById('productLoc').innerHTML = location ? `<i class='fa-solid fa-location-dot'></i> ${location}` : '';
        if(document.getElementById('productDesc')) document.getElementById('productDesc').innerText = '상품 정보를 찾을 수 없습니다.';
        const imgEl = document.getElementById('productImg');
        if (imgEl) {
            imgEl.src = 'https://images.unsplash.com/photo-1511739001486-6bfe10ce785f?auto=format&fit=crop&q=80&w=1200';
            imgEl.alt = title || '';
        }
        PRICES = { adult: 0, child: 0 };
        if(document.getElementById('adultPriceDisplay')) document.getElementById('adultPriceDisplay').innerText = '-';
        if(document.getElementById('childPriceDisplay')) document.getElementById('childPriceDisplay').innerText = '-';
        updateTotalPrice();
    }
}
...
function setupReviewShowMore() {
    ...
    reviewList.forEach((el, idx) => { el.style.display = (idx < 4) ? '' : 'none'; });
    ...
    if (reviewList.length > 4 && moreBtn) {
        ...
        moreBtn.onclick = function() {
            ...
            if (!isOpen) {
                document.querySelectorAll('.review-item').forEach(el => el.style.display = '');
...
function toggleWish() {
    ...
    showToast(active ? '찜 목록에 추가되었습니다!' : '찜 목록에서 제외되었습니다.');
}
