...
const DESTINATION_REGIONS = [
  ...
  },
  {
    ...
    label: "홍콩/마카오/중국",
    countryCode: "CN",
    cities: ["타이베이", "타이중", "가오슝", "홍콩", "마카오", "상하이"],
  },
  {
    key: "pac",
    label: "남태평양",
  ...
  },
  {
    ...
    label: "미주",
    countryCode: "US",
    cities: ["뉴욕", "로스앤젤레스", "라스베이거스", "샌프란시스코"],
  },
  {
    key: "eu",
    label: "유럽",
    countryCode: "FR",
    cities: ["파리", "로마", "런던", "바르셀로나"],
  },
  {
    key: "mea",
    label: "중동/아프리카",
...
function initDestinationPicker(els, state, initial) {
  ...
  const countryCode = String(
    initial.countryCode || els.countryCodeHidden?.value || "JP",
  ).toUpperCase();
  const region =
    DESTINATION_REGIONS.find((r) => r.countryCode === countryCode) ||
...
function renderDestinationRegions(els, state) {
  ...
  els.destinationRegionList.innerHTML = DESTINATION_REGIONS.map(
    (r) => `
  ...
  els.destinationRegionList
    .querySelectorAll(".destination-region-btn")
    .forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        state.destinationRegionKey =
          btn.dataset.region || state.destinationRegionKey;
        state.destinationSearchItems = [];
        state.destinationSearchTried = false;
        state.destinationSearching = false;
...
function renderDestinationCities(els, state) {
  ...
  const hasSearchOverlay =
    state.destinationSearching || state.destinationSearchTried;
  ...
  els.destinationCityGrid.innerHTML = region.cities
    .map(
      (city) => `
  ...
  els.destinationCityGrid
    .querySelectorAll(".destination-city-btn")
    .forEach((btn) => {
...
function bindDestinationSearch(els, state) {
  ...
  if (els.destinationSearchInput)
    els.destinationSearchInput.dataset.bound = "1";
  ...
  els.destinationSearchInput?.addEventListener("click", (e) =>
    e.stopPropagation(),
  );
...
function renderDestinationSearchArea(els, state) {
  ...
  const items = Array.isArray(state.destinationSearchItems)
    ? state.destinationSearchItems
    : [];
  if (!items.length) {
  ...
  listEl.querySelectorAll(".destination-result-item").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      await selectDestinationCity(els, state, {
        ...
        countryCode:
          item.country_code ||
          inferDestinationCountryCode(
            item.name || "",
            getDestinationRegionCountryCode(state),
          ),
      });
    });
  });
...
async function triggerDestinationSearch(els, state) {
  ...
  try {
    ...
    let items = await fetchRentalLocations({
      q: searchQ,
      category: "city",
      countryCode,
    });
    if (!items.length) {
      items = await fetchRentalLocations({
        q: searchQ,
        category: "city",
        countryCode: "",
      });
...
function closeAllPopovers(els) {
  document
    .querySelectorAll(".popover-container")
...
function bindPopoverToggles(els) {
  ...
  document
    ...
    .forEach((b) => b.addEventListener("click", () => closeAllPopovers(els)));
...
function bindLocationSearch(els, state, target) {
  const searchInput =
    target === "pickup" ? els.pickupSearchInput : els.dropoffSearchInput;
  ...
  group.addEventListener("click", () => {
    const fallbackQ = getLocationSearchQuery(
      els,
      target,
...
function getLocationSearchQuery(els, target, typed) {
  return (
    String(typed || "").trim() ||
    (target === "dropoff"
      ? String(els.pickupNameHidden?.value || "").trim()
      : "") ||
...
function renderLocationList(els, target, items, errorMsg) {
  const listEl =
    target === "pickup" ? els.pickupLocationList : els.dropoffLocationList;
  if (!listEl) return;
  if (errorMsg) {
...
function syncDateDisplays(els) {
  const startVal =
    `${els.startDate.value || ""} ${els.startTime.value || ""}`.trim();
...
function rentalSavedRowKey(row) {
  ...
  if (String(row.item_type || "").toLowerCase() === "rental")
    return rentalKeyFromPayload(payload);
...
async function loadRentalSavedState() {
  try {
    const data = await rentalSavedApi("/api/saved-items", {
      method: "GET",
      headers: {},
    });
    rentalSavedState = {
    ...
    if (e?.code !== "LOGIN_REQUIRED")
      console.warn("rental saved-items load failed", e);
...
function updateRentalCardButtons() {
  const wishRows = Array.isArray(rentalSavedState.wishlist)
    ? rentalSavedState.wishlist
    : [];
  const cartRows = Array.isArray(rentalSavedState.cart)
    ? rentalSavedState.cart
    : [];
  ...
  document.querySelectorAll("[data-rental-card]").forEach((card) => {
    ...
    const payload = parseRentalPayload(
      wishBtn?.getAttribute("data-rental-wish") ||
        cartBtn?.getAttribute("data-rental-cart") ||
        "",
    );
...
function renderRentalSavedDrawer() {
  ...
  const total =
    (rentalSavedState.cart?.length || 0) +
    (rentalSavedState.wishlist?.length || 0) +
    (rentalAlertState?.length || 0);
  if (countEl) {
  ...
  const items = Array.isArray(rentalSavedState[rentalSavedTab])
    ? rentalSavedState[rentalSavedTab]
    : [];
  listEl.innerHTML = "";
  emptyEl.style.display = items.length ? "none" : "block";
  emptyEl.textContent =
    rentalSavedTab === "wishlist"
      ? "위시리스트 항목이 없습니다."
      : "장바구니 항목이 없습니다.";
...
async function toggleRentalSaved(rawPayload, listType) {
  ...
  const targetList = Array.isArray(rentalSavedState[listType])
    ? rentalSavedState[listType]
    : [];
  const key = rentalKeyFromPayload(payload.payload);
  const exists = targetList.find((row) => rentalSavedRowKey(row) === key);
  try {
    if (exists) {
      await rentalSavedApi(`/api/saved-items/${Number(exists.id)}`, {
        method: "DELETE",
        headers: {},
      });
    } else {
      await rentalSavedApi("/api/saved-items", {
        method: "POST",
...
function initRentalSavedUi() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (reserveBtn) {
      if (!(await ensureRentalLoggedIn())) return;
      const payload = parseRentalPayload(
        reserveBtn.getAttribute("data-rental-reserve"),
      );
      if (!payload) return;
    ...
    if (alertRemoveBtn) {
      const requestId = Number(
        alertRemoveBtn.getAttribute("data-rental-alert-remove"),
      );
      fetch(`/api/group-buy/join-requests/${requestId}`, {
        method: "DELETE",
        credentials: "include",
      })
        .then(async (res) => {
    ...
    if (removeBtn) {
      ...
      rentalSavedApi(`/api/saved-items/${itemId}`, {
        method: "DELETE",
        headers: {},
      })
        .then(async () => {
